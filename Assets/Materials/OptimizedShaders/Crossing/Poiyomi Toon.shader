Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/57787bd785d76874ab40b839e6d42cf9"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#ifdef POI_MATCAP0_CUSTOM_NORMAL
				#if defined(PROP_MATCAP0NORMALMAP) || !defined(OPTIMIZER_ENABLED)
				normal0 = calculateNormal(poiMesh.normals[1.0], poiMesh, _Matcap0NormalMap, float4(350,24,0,0), float4(0,0,0,0), 0.0, 0.42);
				#endif
				#endif
				#if defined(PROP_MATCAP) || !defined(OPTIMIZER_ENABLED)
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float2 mainUV = poiUV(poiMesh.uv[0.0].xy, _MainTex_ST);
				if (0.0)
				{
				...
				if (1.0)
				{
					...
					poiFragData.finalColor *= lerp(poiThemeColor(poiMods, float4(1,0,0,0).rgb, 0.0), poiThemeColor(poiMods, float4(1,1,1,1).rgb, 0.0), smoothstep(1.0, 2.0, distance(position, poiCam.worldPos)));
					if (0.0)
					{
						poiFragData.finalColor = lerp(poiFragData.finalColor * float4(1,0,0,0).rgb, poiFragData.finalColor, saturate(poiMesh.isFrontFace));
					}
				}
				#ifndef POI_PASS_BASETWO
